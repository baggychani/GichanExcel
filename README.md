...
| 파일 | 설명 |
|------|------|
| **Setup (설치형)** | `GichanExcel_*_x64-setup.exe` — Windows에 설치합니다. xlsx / xls / csv 파일을 더블클릭으로 바로 열 수 있습니다. |

---

## 주요 기능

- **파일 열기** — `.xlsx`, `.xls`, `.csv`, `.tsv`/`.txt`
- **파일 저장** — `.xlsx`, `.csv`, `.tsv`/`.txt` (구형 `.xls` 저장은 지원하지 않으며, 열기 시 자동으로 `.xlsx` 형식으로 안내합니다)
...
- **자동저장 + 종료 확인** — 편집 중 몇 초마다 임시 저장되며, 저장하지 않은 채 닫으려 하면 저장 여부를 먼저 물어봅니다
...
4. 다른 버전으로 올릴 때만 직접 입력 (예: `1.0.1`) 후 실행
